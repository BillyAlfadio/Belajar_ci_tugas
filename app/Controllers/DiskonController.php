 ...
 public function update($id)
{
    ...
    // Cek apakah data diskon dengan ID ini memang ada
    $diskon = $this->diskon->find($id);
    if (!$diskon) {
        return redirect()->to('/diskon')->with('error', 'Data diskon tidak ditemukan.');
    }

    // Cek apakah tanggal yang dimasukkan sudah digunakan oleh diskon lain
    ...
    if ($existing) {
        return redirect()->back()
                         ->withInput()
                         ->with('error', 'Tanggal diskon sudah digunakan!');
    }
